...
abstract class Icmp {
    ...
    abstract fun setsocketRecvTimeout(
        fd: FileDescriptor,
        timeoutMS: Long,
    )
     ...
     */
    protected fun resolveInetAddressWithTimeout(
        host: String,
        timeoutMS: Long = 1000,
    ): InetAddress {
        return runBlocking {
    ...
    suspend fun ping(
        host: String,
        resolveTimeoutMS: Long = 1000,
        pingTimeoutMS: Long = 1000,
        id: UShort = 0u,
        sequence: UShort = 0u,
    ...
    suspend fun openAndPrepareSocket(
        inetAddress: InetAddress,
        timeoutMS: Long = 1000,
    ): FileDescriptor {
    ...
    fun ping(
        ...
        val icmpHeader =
            if (inetAddress is Inet4Address) {
                ...
                val source = InetAddress.getByName("::") as Inet6Address
                val destination = inetAddress as Inet6Address
                IcmpV6EchoPacket(source, destination, id = id, sequence = sequence, data = data)
            }
        ...
        try {
            ...
            val response =
                if (inetAddress is Inet4Address) {
                    IcmpV4Header.fromStream(recvBuffer, bytesRecieved)
                } else {
                    val source = inetAddress as Inet6Address
                    val destination = InetAddress.getByName("::") as Inet6Address
                    IcmpV6Header.fromStream(source, destination, recvBuffer, bytesRecieved)
                }
            ...
            if (response is IcmpV4EchoPacket) {
                if (response.sequence != sequence) {
                    return com.jasonernst.icmp.common.PingResult
                        .Failed("Sequence number mismatch")
                }
                ...
                return com.jasonernst.icmp.common.PingResult
                    .Failed("Unknown ICMP response type")
            }
            return com.jasonernst.icmp.common.PingResult
                .Success(sequence.toInt(), response.size(), responseTimeMs, inetAddress)
        } catch (e: Exception) {
            return com.jasonernst.icmp.common.PingResult
                .Failed(e.message ?: "Failed to ping ${inetAddress.hostAddress}")
        }
    ...
    suspend fun ping(
        ...
        id: UShort = 0u,
        startingSequence: UShort = 0u,
        data: ByteArray = ByteArray(0),
        count: Int? = null,
    ): Flow<com.jasonernst.icmp.common.PingResult> {
        val fd = openAndPrepareSocket(inetAddress, timeoutMS)
        ...
        return callbackFlow {
            ...
            while (count == null || packetsTransmitted < count) {
                ...
                packetsTransmitted++
                send(result)
                if (result is com.jasonernst.icmp.common.PingResult.Success) {
