...
*/
extern "C"
JNIEXPORT jobject JNICALL Java_com_jasonernst_icmp_1lib_ICMPLinux_socket(JNIEnv *env, jclass _ignore, jint domain, jint type, jint protocol) {
    jfieldID field_fd;
...
extern "C"
JNIEXPORT jint JNICALL Java_com_jasonernst_icmp_1lib_ICMPLinux_setsockoptInt(JNIEnv *env, jclass _ignore, jobject fileDescriptor, jint level, jint optname, jint optval) {
    int fd = getFDFromFileDescriptor(env, fileDescriptor);
    return setsockopt(fd, level, optname, &optval, sizeof(optval));
}

extern "C"
JNIEXPORT jint JNICALL Java_com_jasonernst_icmp_1lib_ICMPLinux_setsocketRecvTimeout(JNIEnv *env, jclass _ignore, jobject fileDescriptor, jlong sec, jlong usec) {
    struct timeval tv;
    tv.tv_sec = sec;
    tv.tv_usec = usec;
    int fd = getFDFromFileDescriptor(env, fileDescriptor);
...
extern "C"
JNIEXPORT jint JNICALL Java_com_jasonernst_icmp_1lib_ICMPLinux_sendTo(JNIEnv *env, jclass _ignore, jobject fileDescriptor, jbyteArray data, jint flags, jbyteArray address, jint port) {
...
extern "C"
JNIEXPORT jint JNICALL Java_com_jasonernst_icmp_1lib_ICMPLinux_recvFrom(JNIEnv *env, jclass _ignore, jobject fileDescriptor, jbyteArray data, jint flags, jbyteArray address, jint port) {
    struct sockaddr_in addr;
    jbyte *data_ptr, *addr_ptr;
    jsize data_len, addr_len;
    int ret;
